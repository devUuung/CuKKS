...
### `poly_mod_degree` 선택 가이드

| `poly_mod_degree` | 슬롯 수 | 곱셈 깊이 | 속도 | 사용 사례 |
|-------------------|--------|----------|------|----------|
| 8192 | 4096 | ~3-4 | 빠름 | 얕은 네트워크 (1-2 레이어) |
| 16384 | 8192 | ~6-8 | 보통 | 대부분의 CNN/MLP 모델 |
| 32768 | 16384 | ~12-15 | 느림 | 깊은 네트워크, Transformer |

- **슬롯 수** = 암호문당 최대 요소 수 (SIMD 배치 크기)
- **곱셈 깊이** = 부트스트래핑 없이 연속 곱셈 가능 횟수
- 값이 클수록 메모리 사용량도 증가
- **보안 레벨은 별도 설정** — `security_level` 파라미터로 암호화 강도 지정 (128/192/256비트). 보안 레벨이 높을수록 같은 `poly_mod_degree`에서 사용 가능한 곱셈 깊이 감소
