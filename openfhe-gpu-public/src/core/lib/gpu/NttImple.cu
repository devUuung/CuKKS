...
void Ntt8PointPerThreadPhase1ExcludeSomeRange(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (N / 8 * num_prime);
    ...
    const uint64_t *prime_table = primes;
    // assert(prime_inds[np_idx] == np_idx);
    const uint64_t *W = base_inv + N * prime_inds[np_idx];
    const uint64_t *W_ = base_inv_ + N * prime_inds[np_idx];
    uint64_t prime = prime_table[prime_inds[np_idx]];
...
void Ntt8PointPerThreadPhase2FusedWithSubNegateConstMult(
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (N / 8 * num_prime);
    ...
    for (int j = 0; j < 2; j++) {
      butt_ntt_local(local[4 * j], local[4 * j + 2], W[2 * tw_idx + j], W_[2 * tw_idx + j], prime);
      butt_ntt_local(local[4 * j + 1], local[4 * j + 3], W[2 * tw_idx + j], W_[2 * tw_idx + j], prime);
    }
    for (int j = 0; j < 4; j++) {
      butt_ntt_local(local[2 * j], local[2 * j + 1], W[4 * tw_idx + j], W_[4 * tw_idx + j], prime);
    }
    for (int j = 0; j < 8; j++) {
      temp[set * 8 * radix + t_idx + t / 4 * j] = local[j];
    }
...
#pragma unroll
    for (int j = 8, k = t / 8; j < t / 4 + 1; j *= 8, k >>= 3) {
      ...
      for (int j2 = 0; j2 < 2; j2++) {
        butt_ntt_local(local[4 * j2], local[4 * j2 + 2], W[2 * tw_idx2 + j2], W_[2 * tw_idx2 + j2], prime);
        butt_ntt_local(local[4 * j2 + 1], local[4 * j2 + 3], W[2 * tw_idx2 + j2], W_[2 * tw_idx2 + j2], prime);
      }
      for (int j2 = 0; j2 < 4; j2++) {
        butt_ntt_local(local[2 * j2], local[2 * j2 + 1], W[4 * tw_idx2 + j2], W_[4 * tw_idx2 + j2], prime);
      }
    ...
    if (tail == 1) {
      ...
      butt_ntt_local(local[2], local[3], W[2 * tw_idx2 + 1], W_[2 * tw_idx2 + 1], prime);
      butt_ntt_local(local[4], local[5], W[2 * tw_idx2 + 2], W_[2 * tw_idx2 + 2], prime);
      butt_ntt_local(local[6], local[7], W[2 * tw_idx2 + 3], W_[2 * tw_idx2 + 3], prime);
      for (int l = 0; l < 8; l++) {
        temp[set * 8 * radix + 8 * t_idx + l] = local[l];
      }
    }
