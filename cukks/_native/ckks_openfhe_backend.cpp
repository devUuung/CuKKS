...
std::shared_ptr<CiphertextHandle> repeated_block_matmul_cipher(
    const std::shared_ptr<CiphertextHandle>& tensor,
    const std::vector<std::vector<double>>& block_weight,
    std::uint32_t num_blocks,
    std::uint32_t total_in
) {
...
std::shared_ptr<CiphertextHandle> packed_self_attention_power_cipher(
    const std::shared_ptr<CiphertextHandle>& query,
    const std::shared_ptr<CiphertextHandle>& key,
    const std::shared_ptr<CiphertextHandle>& value,
    std::uint32_t batch_size,
    std::uint32_t seq_len,
    std::uint32_t embed_dim,
    double scale,
    ...
    std::map<int, std::shared_ptr<CiphertextHandle>> rotated_k{{0, key}};
    std::map<int, std::shared_ptr<CiphertextHandle>> rotated_v{{0, value}};
    std::shared_ptr<CiphertextHandle> combined;
    ...
    for (std::uint32_t query_index = 0; query_index < seq_len; ++query_index) {
        ...
        std::vector<std::shared_ptr<CiphertextHandle>> scores;
        ...
        std::vector<std::shared_ptr<CiphertextHandle>> squared;
        squared.reserve(scores.size());
        std::shared_ptr<CiphertextHandle> z_sum;
        ...
        std::vector<std::shared_ptr<CiphertextHandle>> weights;
        weights.reserve(squared.size());
        for (const auto& sq : squared) {
            auto weight = rescale_cipher(mul_cipher(sq, inv_z_sum));
            weights.push_back(weight);
        }
        std::shared_ptr<CiphertextHandle> token_output;
        for (std::uint32_t key_index = 0; key_index < seq_len; ++key_index) {
...
PYBIND11_MODULE(ckks_openfhe_backend, m) {
    ...
    m.def("create_context", &create_context, py::arg("poly_mod_degree"), py::arg("coeff_mod_bits"),
          ...
          py::arg("batch_size"), py::call_guard<py::gil_scoped_release>());
