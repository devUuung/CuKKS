...
class TestTTLinearEmptyValidation:
    """Bug: tt_linear.py crashed with IndexError on empty tt_cores.
    
    Direct instantiation with empty tt_cores would crash at cores_4d[0][0].
    Fix: Validate tt_cores and tt_shapes are non-empty in __init__.
    """

    def test_empty_tt_cores_raises(self):
        """Empty tt_cores should raise ValueError."""
        from ckks_torch.nn import EncryptedTTLinear
        
        with pytest.raises(ValueError, match="tt_cores cannot be empty"):
            EncryptedTTLinear(
                in_features=64,
                out_features=64,
                tt_cores=[],
    ...
    def test_empty_tt_shapes_raises(self):
        """Empty tt_shapes should raise ValueError."""
        from ckks_torch.nn import EncryptedTTLinear
        
        core = torch.randn(1, 16, 4)
        
        with pytest.raises(ValueError, match="tt_shapes cannot be empty"):
            EncryptedTTLinear(
                in_features=64,
                out_features=64,
                tt_cores=[core],
    ...
    def test_mismatched_cores_shapes_raises(self):
        """Mismatched tt_cores and tt_shapes lengths should raise ValueError."""
        from ckks_torch.nn import EncryptedTTLinear
        
        cores = [torch.randn(1, 16, 4), torch.randn(4, 16, 1)]
        shapes = [(4, 4)]
...
class TestTTConv2dEmptyValidation:
    """Bug: tt_conv.py lacked empty list validation like tt_linear.py.
    
    Direct instantiation with empty tt_cores_per_group or tt_shapes would crash
    with IndexError during weight reconstruction.
    Fix: Validate tt_cores_per_group and tt_shapes are non-empty in __init__.
    """

    def test_empty_tt_cores_per_group_raises(self):
        """Empty tt_cores_per_group should raise ValueError."""
        from ckks_torch.nn import EncryptedTTConv2d
        
        with pytest.raises(ValueError, match="tt_cores_per_group cannot be empty"):
            EncryptedTTConv2d(
                in_channels=8,
                out_channels=16,
                kernel_size=(3, 3),
    ...
    def test_empty_tt_shapes_raises(self):
        ...
        with pytest.raises(ValueError, match="tt_shapes cannot be empty"):
            EncryptedTTConv2d(
                ...
                out_channels=16,
                kernel_size=(3, 3),
                tt_cores_per_group=[[core]],
                tt_shapes=[],
            )

    def test_empty_group_cores_raises(self):
        """Empty cores for a specific group should raise ValueError."""
        from ckks_torch.nn import EncryptedTTConv2d
        
        with pytest.raises(ValueError, match="tt_cores for group 0 cannot be empty"):
            EncryptedTTConv2d(
                in_channels=8,
                out_channels=16,
                kernel_size=(3, 3),
