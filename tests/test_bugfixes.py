...
class TestScaleMismatchInAddSub:
    ...
    def test_chained_operations_with_mixed_rescale(self, mock_enc_context):
        ...
        decrypted = mock_enc_context.decrypt(result)
        expected = (
            torch.tensor([1.0, 2.0]) * 2 + torch.tensor([3.0, 4.0]) * 3 - torch.tensor([0.5, 0.5])
        )
...
class TestSoftmaxSeqLen1:
    ...
    def test_softmax_single_element_returns_one(self, mock_enc_context):
        ...
        torch.testing.assert_close(decrypted, torch.ones(4), rtol=1e-4, atol=1e-4)
...
class TestConvMemoryExplosion:
    ...
    def test_small_conv_works(self, mock_enc_context):
        ...
        conv = EncryptedConv2d(
            in_channels=1, out_channels=4, kernel_size=3, weight=weight, stride=1, padding=1
        )
    ...
    def test_compact_conv_matches_dense(self, mock_enc_context):
        ...
        result_compact = conv._forward_he_packed_compact(x_compact, num_patches, patch_features)
...
class TestCNNRotationKeyPlanning:
    ...
    def test_for_model_includes_conv_rotations(self):
        ...
        ctx = CKKSInferenceContext.for_model(model, input_shape=(1, 1, 28, 28))
